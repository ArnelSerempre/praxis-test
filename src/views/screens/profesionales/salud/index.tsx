...
const Salud = () => {
  ...
  return (
    <React.Fragment>
      <React.Fragment>
        <StyledSection4
          background={cuidadores && cuidadores.fondso_salud.content}
        >
          <ContainerStyled>
            ...
            <Grid item lg={12} className="xl:pb-16 d-flex justify-center">
              <video
                src="https://api.praxispharmaceutical.com.co/wp-content/uploads/2022/06/video-corto-mecanismo-de-accion.mp4"
                width="80%"
                controls
              ></video>
            </Grid>
            <div className="my-4" id="accordions">
              {saludArray &&
                saludArray.map((item: any, index: number) => (
                  <Accordion
                    name={item.name}
                    id={item.id}
                    title={item.title}
